...
#   make test     build + run all correctness checks (padic + tree + diffusion + montecarlo)
...
.PHONY: all test walk diffuse mc clean
all: test
...
test: build/test_padic build/test_tree build/test_diffusion build/test_montecarlo  ## build + run every check
	./build/test_padic
	@echo
	./build/test_tree
	@echo
